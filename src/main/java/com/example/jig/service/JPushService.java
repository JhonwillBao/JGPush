...
import com.example.jig.config.JPushConfig;
...
//		alertjson.addproperty("title", pushbean.gettitle());
		alertJSON.addProperty("subtitle", /*"这里是副标题"*/pushBean.getSubtitle());
		alertJSON.addProperty("body", pushBean.getAlert());
		return sendPush(PushPayload.newBuilder()
				.setPlatform(Platform.ios())
				.setAudience(Audience.registrationId(registids))
				.setNotification(Notification.newBuilder().addPlatformNotification(((cn.jpush.api.push.model.notification.IosNotification.Builder) IosNotification.newBuilder().setAlert(alertJSON).autoBadge().setContentAvailable(false).setMutableContent(true).addExtras(pushBean.getExtras())).build()).build())
				.setOptions(Options.newBuilder().setApnsProduction(jPushConfig.getApns()).build())
				.build());
	}
